 ...
 * Integer expression 
 **/
class IntegerExpression : public Expr {
  public:
    IntegerExpression(uint64_t n) : Expr(Kind::INTEGER), n_(n) {}

    uint64_t GetInteger() const { return n_; }
  private: 
    uint64_t n_;
};
...
private:
  /// Condition for the if body.
  std::shared_ptr<Expr> cond_;
  /// Expression to be executed in the main branch body.
  std::shared_ptr<Stmt> stmt_;
