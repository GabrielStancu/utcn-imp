...
private:
  ...
  public:
    ...
    virtual void RegisterLocal(const std::string &name, uint32_t index) = 0;
    virtual int CountLocals() = 0;
  ...
  public:
    ...
    void RegisterLocal(const std::string &name, uint32_t index){}
    int CountLocals() { return 0; }
  ...
  public:
    ...
    Binding Lookup(const std::string &name) const override;
    void RegisterLocal(const std::string &name, uint32_t index) {
      locals_.insert(std::pair<std::string, uint32_t>(name, index));
    }
    int CountLocals() {
      return locals_.size();
    }
...
private:
  ...
  void LowerIntegerExpr(const Scope &scope, const IntegerExpression &expr);
