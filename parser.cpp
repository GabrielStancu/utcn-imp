...
std::shared_ptr<LetStmt> Parser::ParseLetStmt()
{
  ...
  std::string identName(Expect(Token::Kind::IDENT).GetIdent());
  Expect(Token::Kind::COLON);
  std::string typeName(Expect(Token::Kind::IDENT).GetIdent());
  Expect(Token::Kind::EQUAL);
  lexer_.Next();
  auto assignExpr = ParseExpr();

  return std::make_shared<LetStmt>(identName, typeName, assignExpr);
}
...
std::shared_ptr<Expr> Parser::ParseEqualGreaterLowerExpr()
{
  ...
  while (Current().Is(Token::Kind::GREATER) || Current().Is(Token::Kind::GREATER_EQUAL) || 
    ...
    if (Current().Is(Token::Kind::GREATER)) {
      ...
      lexer_.Next();
      auto rhs = ParseAddSubExpr();
      term = std::make_shared<BinaryExpr>(BinaryExpr::Kind::EQUAL_EQUAL, term, rhs);
    }
